...
class Risk(str, Enum):
    READ='read'; WRITE='write'; EXTERNAL='external'; FINANCIAL='financial'; SECURITY='security'
...
class ToolPolicy:
    ...
    def can_use(self, capability: str, confirmed: bool = False) -> bool:
        allowed = {
            'files': self.allow_files, 'shell': self.allow_shell,
            'network': self.allow_network, 'deploy': self.allow_deploy,
            'security_testing': self.allow_security_testing,
            'financial': self.allow_financial,
        }.get(capability, False)
        risk = {'network': Risk.EXTERNAL, 'deploy': Risk.EXTERNAL,
                'security_testing': Risk.SECURITY, 'financial': Risk.FINANCIAL}.get(capability)

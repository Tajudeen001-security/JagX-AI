...
class MovieSpec:
    ...
    def validate(self):
        if not 1 <= self.duration_seconds <= 1200: raise ValueError('duration must be 1..1200 seconds')
        if self.fps not in (24,25,30,48,60): raise ValueError('unsupported fps')
...
class MoviePipeline:
    ...
    def __init__(self, scene_generator, frame_generator, video_generator):
        self.scene_generator=scene_generator; self.frame_generator=frame_generator; self.video_generator=video_generator

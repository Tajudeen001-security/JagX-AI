...
class BoundedCache:
    ...
    def __init__(self, max_entries: int = 1024):
        ...
        self.max_entries=max_entries; self._data=OrderedDict(); self._lock=RLock()
    def get(self,key,default=None):
        with self._lock:
            entry=self._data.get(key)
            if entry is None: return default
            entry.hits += 1; self._data.move_to_end(key); return entry.value
    def put(self,key,value):
        with self._lock:
            self._data[key]=CacheEntry(value); self._data.move_to_end(key)

...
class RequestLimits:
    ...
    def validate_text(self, input_tokens: int, output_tokens: int):
        if not 0 <= input_tokens <= self.max_input_tokens:
            raise ValueError("input token limit exceeded")
        if not 0 <= output_tokens <= self.max_output_tokens:
            raise ValueError("output token limit exceeded")

    def validate_video(self, duration_seconds: int):
        if not 0 <= duration_seconds <= self.max_video_seconds:
            raise ValueError("video request limit exceeded")

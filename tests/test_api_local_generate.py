...
from api.local_generate import make_generate_fn
...
def _tok(tmpdir: Path) -> JagXTokenizer:
    corpus = tmpdir / "c.txt"
    corpus.write_text("hello jagx api generate\n" * 20, encoding="utf-8")
...
def test_make_generate_fn_local():
    cfg = ModelConfig(
        vocab_size=120,
        max_seq_len=32,
        d_model=32,
    ...
    with tempfile.TemporaryDirectory() as d:
        tok = _tok(Path(d))
        gen = make_generate_fn(model, tok)
        out = gen({"prompt": "hello", "max_tokens": 3, "temperature": 1.0, "top_k": 10})
        assert out["backend"] == "local-jagx"
        assert out["external_ai_api_required"] is False
        assert isinstance(out["text"], str)

        Handler = create_app(generate_fn=gen)
        out2 = Handler.post_routes["/v1/generate"]({"prompt": "hi", "max_tokens": 2})
        assert out2["backend"] == "local-jagx"

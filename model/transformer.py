...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        s=s.masked_fill(self.mask[:,:,:t,:t]==0,torch.finfo(s.dtype).min)
        a=self.dropout(torch.softmax(s,dim=-1))
        return self.out((a@v).transpose(1,2).contiguous().view(b,t,c))
...
class JagXTransformer(nn.Module):
    ...
    def forward(self,input_ids,labels=None):
        ...
        x=self.token_embedding(input_ids)+self.position_embedding(p)[None,:,:]
        for block in self.blocks: x=block(x)
        logits=self.lm_head(self.norm(x)); loss=None
        if labels is not None: loss=nn.functional.cross_entropy(logits.reshape(-1,logits.size(-1)),labels.reshape(-1))
        return logits,loss
    @torch.no_grad()

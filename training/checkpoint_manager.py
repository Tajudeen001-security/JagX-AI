...
class CheckpointManager:
    ...
    def record(self, step: int, path: str, eval_score: float, loss: float):
        ...
        if not all(map(lambda x: isinstance(x, (int, float)), (eval_score, loss))):
            raise TypeError("scores must be numeric")
        records = self.records()
        records.append(CheckpointRecord(step, str(path), float(eval_score), float(loss)))
    ...
    def best(self):
        records = self.records()
        return max(records, key=lambda r: r.eval_score) if records else None

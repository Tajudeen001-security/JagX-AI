...
def save_checkpoint(path: str, model, optimizer, scheduler=None, step: int = 0, metadata: dict[str, Any] | None = None):
    ...
    torch.save({'step':step,'model':model.state_dict(),'optimizer':optimizer.state_dict(),'scheduler':scheduler.state_dict() if scheduler else None,'metadata':metadata or {}},target)


def load_checkpoint(path: str, model, optimizer=None, scheduler=None, map_location='cpu') -> int:
    state=torch.load(path,map_location=map_location,weights_only=True)
    required={'step','model'}
    if not required.issubset(state): raise ValueError('invalid JagX checkpoint: missing required fields')

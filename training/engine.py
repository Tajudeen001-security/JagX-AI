...
@dataclass
class TrainConfig:
    steps:int=1000; grad_accum:int=1; lr:float=3e-4; min_lr:float=3e-5
    warmup_steps:int=100; max_grad_norm:float=1.0; log_every:int=10
    checkpoint_every:int=500; checkpoint_path:str="checkpoints/latest.pt"; seed:int=42

class Trainer:
    def __init__(self,model,optimizer,config:TrainConfig,scheduler=None):
        self.model=model; self.optimizer=optimizer; self.config=config; self.scheduler=scheduler
    ...
    def fit(self,batches:Iterable[tuple[torch.Tensor,torch.Tensor]],start_step:int=0):
        ...
        for step in range(start_step,self.config.steps):
            self.optimizer.zero_grad(set_to_none=True); total_loss=0.0
            ...
            last_step=step+1
            if step%self.config.log_every==0: print(f"step={step} loss={total_loss:.4f} lr={lr:.6g}")
            if self.config.checkpoint_every and last_step%self.config.checkpoint_every==0:
                save_checkpoint(self.config.checkpoint_path,self.model,self.optimizer,self.scheduler,last_step,{"config":self.config.__dict__})
        return last_step
